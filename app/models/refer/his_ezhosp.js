...
class HisEzhospModel {
    ...
    getDiagnosisOpd(db, visitno, hospCode = hcode) {
        return db('view_opd_dx as dx')
            ...
            .select('dx.hn as pid', 'dx.vn as seq', 'dx.type as diagtype', 'dx.diag as diagcode', 'dx.desc as diagname', 'dx.clinic_std as clinic', 'dx.dr_dx as provider', 'patient.no_card as cid')
    ...
    getAdmission(db, columnName, searchNo, hospCode = hcode) {
        ...
        return db('view_ipd_ipd as ipd')
            ...
            .select('ipd.ward_std as warddisch', 'ipd.dischstatus', 'ipd.dischtype', 'ipd.price', 'ipd.paid as payprice')
            .select(db.raw('0 as actualpay'))
            .select('ipd.dr_disc as provider')
            .select(db.raw('concat(ipd.disc, " " , ipd.timedisc) as d_update'))
            .select('ipd.drg', 'ipd.rw', 'ipd.adjrw', 'ipd.drg_error as error', 'ipd.drg_warning as warning', 'ipd.los as actlos', 'ipd.grouper_version', 'ipd.no_card as cid')
            .where(columnName, "=", searchNo)
            .limit(maxLimit);
    }
    getDiagnosisIpd(db, columnName, searchNo, hospCode = hcode) {
        ...
        return db('view_ipd_dx as dx')
            ...
            .select('clinic_std as warddiag', 'dx as diagcode', 'type as diagtype', 'dr as provider', 'dx.desc as diagname', 'cid', 'lastupdate as d_update')
            .where(columnName, "=", searchNo)
            .orderBy('an')
            .orderBy('type')
            .limit(maxLimit);
...
//# sourceMappingURL=his_ezhosp.js.map